...
function Fiche({ctx}){
  ...
  const vts=ctx.cv(c.id);const hrs=ctx.ch(c.id);const sol=ctx.cs(c.id);
  const famData=ctx.csFamille(c.id);
  const[ficheTab,setFicheTab]=useState("ventes");
  // Heures history for this cavalier
  const heurHist=ctx.hm.filter(h=>h.cav===c.id).sort((a,b)=>b.date.localeCompare(a.date));

  return<div style={{padding:"0 16px"}}>
    {/* Identity - always same */}
    <div style={S.card}>
      ...
      {/* Always same info block: child shows birthday+payeur, parent shows tel+email+birthday */}
      {!isP&&<>{c.naissance&&<IR l="Né(e) le" v={fD(c.naissance)}/>}{py&&<IR l="Payeur" v={`${py.prenom} ${py.nom}`}/>}</>}
      {isP&&<>{c.tel&&<IR l="Tél" v={c.tel}/>}{c.email&&<IR l="Email" v={c.email}/>}{c.naissance&&<IR l="Né(e) le" v={fD(c.naissance)}/>}</>}
    </div>

    {/* 2 stat boxes - ALWAYS same layout */}
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:12}}>
      <div style={S.stBx}>
        <div style={S.stLb}>Reste à payer</div>
        <div style={{...S.stVl,color:sol>0?"#e87a7a":"#5ae8a0"}}>{fE(sol)}</div>
        {/* Family detail inside the box for parents */}
        {isP&&famData.details.length>1&&<div style={{marginTop:8,borderTop:"1px solid rgba(212,175,105,.08)",paddingTop:6}}>
          {famData.details.map(d=><div key={d.id} style={{display:"flex",justifyContent:"space-between",padding:"2px 0"}}>
            <span style={{color:"#7a6f60",fontSize:10}}>{d.nom.split(" ")[0]}</span>
            <span style={{color:d.solde>0?"#e87a7a":"#5ae8a0",fontSize:10,fontWeight:600}}>{fE(d.solde)}</span>
          </div>)}
        </div>}
      </div>
      <div style={S.stBx}>
        ...
        <div style={{fontSize:11,color:"#7a6f60",marginTop:2}}>{hrs.achats}h achetées · {hrs.consommes}h prises</div>
      </div>
    </div>
    ...
    {/* Ventes / Heures tabs */}
    <div style={S.card}>
      <div style={{display:"flex",gap:4,marginBottom:12,background:"rgba(212,175,105,.06)",borderRadius:10,padding:3}}>
        <button onClick={()=>setFicheTab("ventes")} style={{...S.tabBtn,background:ficheTab==="ventes"?"#d4af69":"transparent",color:ficheTab==="ventes"?"#1a1207":"#7a6f60"}} className="bh">Ventes</button>
        <button onClick={()=>setFicheTab("heures")} style={{...S.tabBtn,background:ficheTab==="heures"?"#d4af69":"transparent",color:ficheTab==="heures"?"#1a1207":"#7a6f60"}} className="bh">Heures</button>
      </div>
...
function Planning({ctx}){
  ...
  return<div style={{padding:"0 16px"}}>
    ...
    <div style={{display:"flex",justifyContent:"flex-end",marginBottom:12}}><button onClick={()=>ctx.setMdl({t:"ncr",jour:j})} style={S.aBtn} className="bh"><Ico n="plus" s={14}/> Créneau</button></div>
    {jc.length===0&&<p style={S.mt}>Aucun créneau le {j.toLowerCase()}</p>}
    {jc.map(cr=>{
      const cavNames=cr.cavs.map(id=>{const c=ctx.gc(id);return c?`${c.prenom} ${c.nom[0]}.`:null}).filter(Boolean).join(", ");
      return<div key={cr.id} style={{...S.card,position:"relative",cursor:"pointer"}} onClick={()=>ctx.setMdl({t:"pr",crid:cr.id})} className="bh">
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:4}}>
          <div><span style={{fontWeight:700,color:"#d4af69",fontSize:18}}>{cr.heure}</span><span style={{color:"#b8a88a",marginLeft:10,fontSize:15}}>{cr.nom}</span></div>
          <button onClick={e=>{e.stopPropagation();ctx.setMdl({t:"ecr",crid:cr.id});}} style={{...S.iBtn,padding:6}} className="bh"><Ico n="edit" s={16} c="#d4af69"/></button>
        </div>
        <div style={{color:"#7a6f60",fontSize:12}}>{cavNames||"Aucun cavalier"}</div>
        <div style={{color:"#5a5040",fontSize:10,marginTop:4}}>Appuyer pour gérer les présences</div>
      </div>;
    })}
  </div>;
}
