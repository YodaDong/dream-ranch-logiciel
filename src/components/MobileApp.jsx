...
function Fiche({ctx}){
  ...
  return<div style={{padding:"0 16px"}}>
    <div style={S.card}>
      ...
      {!isP&&<>{c.naissance&&<IR l="Né(e) le" v={fD(c.naissance)}/>}{py&&<IR l="Parent" v={`${py.prenom} ${py.nom}`}/>}</>}
      {isP&&<>{c.tel&&<IR l="Tél" v={c.tel}/>}{c.email&&<IR l="Email" v={c.email}/>}</>}
    </div>
    ...
    <div style={S.card}>
      ...
      {ficheTab==="heures"&&<>
        ...
        {heurHist.length===0&&ctx.prs.filter(p=>p.cav===c.id&&p.ok).length===0&&<p style={S.mt}>Aucun historique</p>}
        {ctx.prs.filter(p=>p.cav===c.id&&p.ok).sort((a,b)=>b.date.localeCompare(a.date)).map(p=>{const cr=ctx.crs.find(x=>x.id===p.cr);return<div key={p.id} style={{...S.lRow,cursor:"default"}}><div style={{flex:1}}><div style={S.rT}>{cr?.nom||"Cours"}</div><div style={S.rS}>{fD(p.date)} · {cr?.heure||""}</div></div><span style={{fontSize:13,fontWeight:700,color:"#e87a7a"}}>-1h</span></div>})}
        {heurHist.map(h=><div key={h.id} style={{...S.lRow,cursor:"default"}}><div style={{flex:1}}><div style={{...S.rT,color:h.delta>0?"#5ae8a0":"#e87a7a"}}>{h.delta>0?"+":""}{h.delta}h — Ajustement</div><div style={S.rS}>{fD(h.date)}{h.motif?` · ${h.motif}`:""}</div></div></div>)}
      </>}
    </div>
  </div>;
}
...
function Caisse({ctx}){
  ...
  const autoMvts=ctx.vts.flatMap(v=>v.pays.filter(p=>p.mode==="Espèces").map(p=>({id:p.id,date:p.date,mt:p.mt,type:"entree",label:v.detail,auto:true,cav:ctx.gc(v.cav)})));
  const manuelMvts=(ctx.caisseMvts||[]).map(m=>({...m,auto:false}));
  const allMvts=[...autoMvts,...manuelMvts].sort((a,b)=>b.date.localeCompare(a.date));
